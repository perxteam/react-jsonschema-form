...
function validateFormData(formData, schema, customValidate, transformErrors, context, uiSchema) {
  ...
  var errorHandler = customValidate(formData, createErrorHandler(formData), dirtyFields);
  var userErrorSchema = unwrapErrorHandler(errorHandler);
  var newErrorSchema = (0, _utils.mergeObjects)(errorSchema, userErrorSchema, true);
  // XXX: The errors list produced is not fully compliant with the format
  // exposed by the jsonschema lib, which contains full field paths and other
